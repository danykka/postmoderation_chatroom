//TO FIX: 
//	запись в server.log работает через раз;
//	иногда при присоединении пользователя сервер вылетает с ошибкой acception error: Invalid argument;
//	сообщения отправляются вместе с заданной командой, например, пользователь получает: /a "text"
//	/help не реализован!
...
void server_init(int port) //инициализация сервера, создание сокета, инициализация адреса и порта
{
...
void delete_connection(client_connection *p_client) //удаление клиента из списка
{
...
void send_message(client_connection *p_client, char *message_text) { //вывод сообщений
	message msg;
	printf("public message sent by %s:%s\n", p_client->nickname,  message_text);
	strncpy(msg.nickname, p_client->nickname, 20);
...
void handle_client_message(void *p_client) { //функция-хэндлер для приёма сообщений
	int read_size;
	int leave_flag = 0;
